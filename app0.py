...
@app.route('/')
def home():
    item_weight = sorted(data['item_weight'].unique())
    item_visibility = sorted(data['item_visibility'].unique())
    item_mrp = sorted(data['item_mrp'].unique())
    outlet_years = sorted(data['outlet_years'].unique())
    item_fat_content = data['Outlet_Location_Type'].unique()
